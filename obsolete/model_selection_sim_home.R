...
###################################################
#simulate from a gamma mixture model		!!!
###################################################
y <- sim[,1]
X1 <- cbind(1, sim[,3:4])
stat <- sim[,2]
...
run_gamma_mix3 <- cure_rate_MC3_general( 
        ...
        Censoring_status = stat,                
        nChains = nChains,              # number of heated chains (denoted as `C` in the paper)
        mcmc_cycles = 1000,     # number of MCMC cycles (denoted as `N` in the paper)
        nCores = 8,                     # number of cores to use for parallel processing
        sweep = 10,                     # number of iterations per MCMC cycle (denoted  as `m_1` in the paper)
        alpha = alpha,                  # a decreasing sequence of temperatures per chain 
#                                               (starting from 1 - the target chain is the first one). 
...
promotion_time = list(distribution = 'lomax', 
                prior_parameters = matrix(rep(c(2.1, 1.1), 2), byrow = TRUE, 2, 2),
                prop_scale = c(0.1, 0.2)
        )


run_lomax <- cure_rate_MC3_general( 
        y = y, 
        X = X1,
        Censoring_status = stat,                
        nChains = nChains,              # number of heated chains (denoted as `C` in the paper)
...
getSummary_lomax <- compute_map_and_hdis_general(y = y, X = X1, Censoring_status = stat, 
        retained_mcmc = run_lomax$mcmc_sample[-(1:burn), ], prior_parameters = NULL, promotion_time = promotion_time)
...
run_dagum <- cure_rate_MC3_general( 
        y = y, 
        X = X1,
        Censoring_status = stat,                
        nChains = nChains,              # number of heated chains (denoted as `C` in the paper)
...
getSummary_dagum <- compute_map_and_hdis_general(y = y, X = X1, Censoring_status = stat, 
        retained_mcmc = run_dagum$mcmc_sample[-(1:burn), ], prior_parameters = NULL, promotion_time = promotion_time)


# run a logLogistic model (including const. term)

promotion_time = list(distribution = 'logLogistic', 
                prior_parameters = matrix(rep(c(2.1, 1.1), 2), byrow = TRUE, 2, 2),
                prop_scale = c(0.1, 0.2)
        )


run_logLogistic <- cure_rate_MC3_general( 
        y = y, 
        X = X1,
        Censoring_status = stat,                
        nChains = nChains,              # number of heated chains (denoted as `C` in the paper)
...
getSummary_logLogistic <- compute_map_and_hdis_general(y = y, X = X1, Censoring_status = stat, 
        retained_mcmc = run_logLogistic$mcmc_sample[-(1:burn), ], prior_parameters = NULL, promotion_time = promotion_time)


# run a Gompertz model (including const. term)

promotion_time = list(distribution = 'gompertz', 
                prior_parameters = matrix(rep(c(2.1, 1.1), 2), byrow = TRUE, 2, 2),
                prop_scale = c(0.1, 0.2)
        )


run_gompertz <- cure_rate_MC3_general( 
        y = y, 
        X = X1,
        Censoring_status = stat,                
        nChains = nChains,              # number of heated chains (denoted as `C` in the paper)
...
getSummary_gompertz <- compute_map_and_hdis_general(y = y, X = X1, Censoring_status = stat, 
        retained_mcmc = run_gompertz$mcmc_sample[-(1:burn), ], prior_parameters = NULL, promotion_time = promotion_time)
	...
	bics[6] <- getSummary_lomax$bic
	bics[7] <- getSummary_dagum$bic
	bics[8] <- getSummary_logLogistic$bic			
	bics[9] <- getSummary_gompertz$bic				
	names(bics) = c('Exponential', 'Weibull', 'Gamma', 'Gamma_mix_2',  'Gamma_mix_3', 'Lomax', 'Dagum', 'Log-logistic', 'Gompertz')
	...
	latent_cured_status_lomax <- 1-colMeans(run_lomax$latent_status_censored)
	latent_cured_status_dagum <- 1-colMeans(run_dagum$latent_status_censored)
	latent_cured_status_logLogistic <- 1-colMeans(run_logLogistic$latent_status_censored)
	latent_cured_status_gompertz <- 1 - colMeans(run_gompertz$latent_status_censored)
	...
	pred_lomax <- prediction(latent_cured_status_lomax, labels)
	pred_dagum <- prediction(latent_cured_status_dagum, labels)
	pred_logLogistic <- prediction(latent_cured_status_logLogistic, labels)	
	pred_gompertz <- prediction(latent_cured_status_gompertz, labels)		
	...
	perf_lomax <- performance(pred_lomax, "tpr", "fpr")
	perf_dagum <- performance(pred_dagum, "tpr", "fpr")
	perf_logLogistic <- performance(pred_logLogistic, "tpr", "fpr")	
	perf_gompertz <- performance(pred_gompertz, "tpr", "fpr")		
	
	pdf(file = 'img/model_selection_gamma_mix_n1000.pdf', width = 12, height = 5)
	...
	plot(perf_lomax, add = TRUE, col = 6, lty = 6, lwd = 2)	
	plot(perf_dagum, add = TRUE, col = 7, lty = 7, lwd = 2)	
	plot(perf_logLogistic, add = TRUE, col = 8, lty = 8, lwd = 2)				
	plot(perf_gompertz, add = TRUE, col = 9, lty = 9, lwd = 2)					
	...
	auc_lomax <- performance(pred_lomax, measure = "auc")@y.values[[1]]	
	auc_dagum <- performance(pred_dagum, measure = "auc")@y.values[[1]]	
	auc_logLogistic <- performance(pred_logLogistic, measure = "auc")@y.values[[1]]				
	auc_gompertz <- performance(pred_gompertz, measure = "auc")@y.values[[1]]					
	aucs <- c(auc_exp, auc_weibull, auc_gamma, auc_gamma_mix2, auc_gamma_mix3, auc_lomax, auc_dagum, auc_logLogistic, auc_gompertz)
	...
	posterior_probs <- colMeans(run_lomax$latent_status_censored)
	fdr_tpr <- compute_fdr_tpr(true_latent_status, posterior_probs, myCut = myCut)
	my_pch <- def
	contr <- apply(fdr_tpr[,c(1,3)], 1, diff) > 0	
	my_pch[contr] <- def2[contr]
	points(fdr_tpr, pch = my_pch, lwd = 2, cex = 2, type = 'b', col = 6, lty = 6)

	posterior_probs <- colMeans(run_dagum$latent_status_censored)
	fdr_tpr <- compute_fdr_tpr(true_latent_status, posterior_probs, myCut = myCut)
	my_pch <- def
	contr <- apply(fdr_tpr[,c(1,3)], 1, diff) > 0	
	my_pch[contr] <- def2[contr]
	points(fdr_tpr, pch = my_pch, lwd = 2, cex = 2, type = 'b', col = 7, lty = 7)

	posterior_probs <- colMeans(run_logLogistic$latent_status_censored)
	fdr_tpr <- compute_fdr_tpr(true_latent_status, posterior_probs, myCut = myCut)
	my_pch <- def
	contr <- apply(fdr_tpr[,c(1,3)], 1, diff) > 0	
	my_pch[contr] <- def2[contr]
	points(fdr_tpr, pch = my_pch, lwd = 2, cex = 2, type = 'b', col = 8, lty = 8)

	posterior_probs <- colMeans(run_gompertz$latent_status_censored)
	fdr_tpr <- compute_fdr_tpr(true_latent_status, posterior_probs, myCut = myCut)
	my_pch <- def
	contr <- apply(fdr_tpr[,c(1,3)], 1, diff) > 0	
	my_pch[contr] <- def2[contr]
	points(fdr_tpr, pch = my_pch, lwd = 2, cex = 2, type = 'b', col = 9, lty = 9)


	legend('bottomright', 'nominal FDR levels', lty = 2, col = 'gray')
	dev.off()
	
	save.image('model_selection_gamma_mix_n1000.RData')
