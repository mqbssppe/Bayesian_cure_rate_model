...
cure_rate_MC3 <- function( y, X, Censoring_status, nChains = 16, 
				...
				nCores = 8, 
				sweep = 10,
				mu_g = 1, s2_g = 1, a_l = 2.1, b_l = 1, 
				a_1 = 2.1, b_1 = 1, a_2 = 2.1, b_2 = 1, 
	...
	if(nChains < 2){
		...
		run <- cure_rate_metropolis_hastings_cpp(y, X, Censoring_status,  m = mcmc_cycles, alpha = 1,
						...
						a1_prop_scale = a1_prop_scale, 
						a2_prop_scale = a2_prop_scale, 
						b_prop_sd = b_prop_sd, 
...
log_gamma_mixture <- function(y, a1, a2, p, c_under = 1e-9){
	...
	for (k in 1:K){
		...
		log_F_k[,k] <- logp[k] + pgamma(y, shape = a1[k], rate = a2[k], log = TRUE)
	}
...
log_logLogistic <- function(y, a1, a2, c_under = 1e-9){
	...
	log_F <- pllogis(y, shape = a1, scale = a2, log = TRUE)
	ind <- (log_F < c_under)
	if(length(ind) > 0){
		log_F[ind] <- c_under
	}
...
log_gompertz <- function(y, a1, a2, c_under = 1e-9){
	...
	log_F <- pgompertz(y, shape = a1, rate = a2, log = TRUE)
	ind <- (log_F < c_under)
	if(length(ind) > 0){
		log_F[ind] <- c_under
	}
...
log_gamma <- function(y, a1, a2, c_under = 1e-9){
	...
	log_F <- pgamma(y, shape = a1, rate = a2, log = TRUE)
	ind <- (log_F < c_under)
	if(length(ind) > 0){
		log_F[ind] <- c_under
	}
...
log_lomax <- function(y, a1, a2, c_under = 1e-9){
	...
	log_F <- plomax(y, scale = a1, shape3.q = a2, log = TRUE)
	ind <- (log_F < c_under)
	if(length(ind) > 0){
		log_F[ind] <- c_under
	}
...
log_dagum <- function(y, a1, a2, a3, c_under = 1e-9){
	...
	log_F <- pdagum(y, scale = a1, shape1.a = a2, shape2.p = a3, log = TRUE)
	ind <- (log_F < c_under)
	if(length(ind) > 0){
		log_F[ind] <- c_under
	}
...
cure_rate_metropolis_hastings_cpp_general <- function( y, X, Censoring_status,  m, alpha = 1,
...
#		* only positive prior parameters are considered at the moment. 
	...
	if(promotion_time$distribution == 'gamma_mixture'){
		K = promotion_time$K
		if(dim(prior_parameters)[3] != K){stop("incosistent number of mixture components")}		
...
cure_rate_MC3_general <- function( y, X, Censoring_status, nChains = 16, 
				mcmc_cycles = 15000, 
				alpha = NULL, 
				nCores = 8, 
				sweep = 10,
				...
				plot = TRUE,
				adjust_scales = TRUE,
				verbose = TRUE, tau_mala = 0.000015, mala = 0.15, 
	...
	if(nChains < 2){
		...
		run <- cure_rate_metropolis_hastings_cpp_general(y, X, Censoring_status,  m = mcmc_cycles, alpha = 1,
						mu_g = mu_g, s2_g = s2_g, a_l = a_l, b_l = b_l, 
...
#		b_prop_sd[chain_iter,] = b_prop_sd
		...
		iter <- 0
		  ...
		  run <- cure_rate_metropolis_hastings_cpp_general(  y, X, Censoring_status,  
		                        m = 1000, 
				mu_g = mu_g, s2_g = s2_g, a_l = a_l, b_l = b_l, 
...
#				}else{perform_mala = 0}
#			}
			mcmc_chain <- cure_rate_metropolis_hastings_cpp_general(  y, X, Censoring_status,  
				                m = sweep, 
						mu_g = mu_g, s2_g = s2_g, a_l = a_l, b_l = b_l, 
						promotion_time = promotion_time_all[[chain_iter]],
						mu_b = mu_b, Sigma = Sigma,
...
#			print(denom)
			...
			state_j2 <- as.list(parLoop[[j2]]$mcmc_sample[sweep,])
			state_j2[["I_sim_D0"]] <- parLoop[[j2]]$latent_status_censored[sweep,]
			nom1 <- cure_rate_metropolis_hastings_cpp_general(  y, X, Censoring_status,  
								m = 1, # only the log-posterior is computed at the initial values
								mu_g = mu_g, s2_g = s2_g, a_l = a_l, b_l = b_l, 
						...
						mu_b = mu_b, Sigma = Sigma,
								alpha = alpha[j1], plot = FALSE, 
								initialValues = state_j2, tau_mala = tau_mala, mala = mala)
			nom2 <- cure_rate_metropolis_hastings_cpp_general(  y, X, Censoring_status,  
								m = 1, # only the log-posterior is computed at the initial values
								alpha = alpha[j2], plot = FALSE, 
								mu_g = mu_g, s2_g = s2_g, a_l = a_l, b_l = b_l, 
...
#				}
		...
		names(result[[6]]) <- c('y', 'X', 'Censoring_status', 'mu_g', 's2_g', 'a_l', 'b_l', 'mu_b', 'Sigma', 'promotion_time')
	 	names(result) <- c("mcmc_sample", "latent_status_censored", "complete_log_likelihood","swap_accept_rate",'all_cll_values', 'input_data_and_model_prior')
...
#' @export
summary.bayesCureModel <- function(cure_rate_mcmc_object, burn = NULL, gamma_mix = TRUE, K_gamma = 2, main_mode = TRUE){

	if(is.null(burn)){
		burn = floor(dim(cure_rate_mcmc_object$mcmc_sample)[1]/3)
		cat(paste0('By default, I will discard the first one third of the mcmc sample as burn-in period. Alternatively, you may set the "burn" parameter to another value.'),'\n')
	}else{
		if(burn > dim(cure_rate_mcmc_object$mcmc_sample)[1] - 1){stop('burn in period not valid.')}
		if(burn < 0){stop('burn in period not valid.')}		
	}
	y = cure_rate_mcmc_object$input_data_and_model_prior$y
	X = cure_rate_mcmc_object$input_data_and_model_prior$X
	Censoring_status = cure_rate_mcmc_object$input_data_and_model_prior$Censoring_status
	promotion_time = cure_rate_mcmc_object$input_data_and_model_prior$promotion_time
	retained_mcmc = cure_rate_mcmc_object$mcmc_sample[-(1:burn),]
	mu_g = cure_rate_mcmc_object$input_data_and_model_prior$mu_g
	s2_g = cure_rate_mcmc_object$input_data_and_model_prior$s2_g	
	mu_b = cure_rate_mcmc_object$input_data_and_model_prior$mu_b
	Sigma = cure_rate_mcmc_object$input_data_and_model_prior$Sigma
	a_l = cure_rate_mcmc_object$input_data_and_model_prior$a_l
	b_l = cure_rate_mcmc_object$input_data_and_model_prior$b_l
	...
	for(iter in ind){
		...
		if(promotion_time$distribution == 'exponential'){
			lw <- log_weibull(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = 1,  c_under = 1e-9)
		}

		if(promotion_time$distribution == 'weibull'){
			lw <- log_weibull(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = retained_mcmc[iter,'a2_mcmc'],  c_under = 1e-9)
		}

		if(promotion_time$distribution == 'gamma'){
			lw <- log_gamma(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = retained_mcmc[iter,'a2_mcmc'],  c_under = 1e-9)
		}

		if(promotion_time$distribution == 'gompertz'){
			lw <- log_gompertz(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = retained_mcmc[iter,'a2_mcmc'],  c_under = 1e-9)
		}


		if(promotion_time$distribution == 'logLogistic'){
			lw <- log_logLogistic(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = retained_mcmc[iter,'a2_mcmc'],  c_under = 1e-9)
		}

		if(promotion_time$distribution == 'lomax'){
			lw <- log_lomax(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = retained_mcmc[iter,'a2_mcmc'],  c_under = 1e-9)
		}
		if(promotion_time$distribution == 'dagum'){
			lw <- log_dagum(y, a1 = retained_mcmc[iter,'a1_mcmc'], a2 = retained_mcmc[iter,'a2_mcmc'], 
		...
		logS <- log_S_p(g = retained_mcmc[iter,'g_mcmc'], 
			lambda = retained_mcmc[iter,'lambda_mcmc'], 
			log_F = lw$log_F,
			b = retained_mcmc[iter, bIndices], 
			x = x)
...
#			        0.5 * mahalanobis(b, mu_b, Sigma)^2
	...
	hh <- Mclust(logP, G = 1:3)
	ind <- which(hh$classification == hh$G)

	if(main_mode){
		latent_cured_status <- 1 - colMeans(cure_rate_mcmc_object$latent_status_censored[burn + ind,])
		# computation of DIC2 but only on the main mode
		p_D <- -2*mean(logL[ind], na.rm = TRUE) + 2*D_theta_hat
		dic_main <- -2 * D_theta_hat + 2 * p_D
	}else{
		latent_cured_status <- 1 - colMeans(cure_rate_mcmc_object$latent_status_censored[-(1:burn),])
	...
	results[[3]] <- dic2	
	results[[4]] <- dic_main
	results[[5]] <- map_estimate
	results[[6]] <- hdis
	results[[7]] <- latent_cured_status
	names(results) <- c('log_posterior', 'bic','dic', 'dic_main_mode', 'map_estimate', 'highest_density_indervals', 'latent_cured_status')
	return(results)
