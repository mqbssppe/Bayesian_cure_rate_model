...
scenario[[5]] <- c(1, 1, 1, 2, -4, 1, 1, 0.8)
...
source('sim_data_function_new_weibull_gamma.R')
sim <- sim_fotis_model(n = n, truePars = truePars, ab = ab, seed = 1, discrcov = 5, promodis = 1)
...
getSummary_gamma_mix3 <- compute_map_and_hdis_general(y = y, X = X1, Censoring_status = stat, 
	...
	pdf(file = 'img/model_selection_weibull_office_n1000.pdf', width = 12, height = 5)
	...
	plot(perf_exp, lwd = 2, xlim = c(0,0.5), ylim = c(0,1))
	...
	save.image('model_selection_weibull_office_n1000.RData')
