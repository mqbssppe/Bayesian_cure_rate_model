...
run <- cure_rate_MC3( 
	myData = myData, 		# this is the input dataset - should be in the format described above
...
#######################################################################################################
        ...
        Sigma = 10*diag(3),     # beta = (beta_0, beta_1, beta_2) ~ N_3 (0, Sigma)
########################################################################################################	        
...
for(i in 1:7){
	...
	abline(h = truePars[i], col = 'yellow', lwd = 2)
}
